...
def refill_select_finl():
    ...
    if get_payments['way_form'] == "True":
        active_kb.append(InlineKeyboardButton("📋 QIWI форма(Не работает)", callback_data="refill_select:Form"))
    if get_payments['way_number'] == "True":
        active_kb.append(InlineKeyboardButton("📞 QIWI номер(Не работает)", callback_data="refill_select:Number"))
    if get_payments['way_nickname'] == "True":
        active_kb.append(InlineKeyboardButton("👷 Через оператора", callback_data="refill_select:Nickname"))
...
def products_open_finl(position_id, category_id, remover):
    ...
    keyboard.add(
        ...
        InlineKeyboardButton("👷 Оператор", url='https://t.me/willywonka2023') 
    ).add(
...
def user_support_finl(user_name):
    ...
    keyboard.add(
        InlineKeyboardButton("💌 Написать в поддержку", url=f"https://t.me/willywonka2023"),
    )
